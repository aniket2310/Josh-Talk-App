...
class PhotoTaskViewModel(
    ...
    val capturedImage: State<ImageBitmap?> = _capturedImage
    ...
    fun onImageCaptured(bitmap: ImageBitmap?) {
        ...
        imagePath = "photo_${Random.nextLong()}"
    }
    ...
    fun canSubmit(): Boolean {
        val hasImage = _capturedImage.value != null
    ...
    fun submitTask() {
        scope.launch {
            try {
                repository.savePhotoCaptureTask(
                    imagePath = imagePath,
                    audioPath = fileName,
                    durationSec = _recordingDuration.value,
                    description = _descriptionText.value.ifEmpty { null }
                )
                println("Photo Capture Task saved successfully")
            } catch (e: Exception) {
